...
public partial class AttributeDatabaseViewModel : EditorViewModelBase
{
    ...
    private readonly AttributeDatabaseContext _dbContext;

    public AttributeDatabaseViewModel(/*MainViewModel mainViewModel,*/ IDialogService dialogService, AttributeDatabaseContext dbContext)
    {
    ...
    public async Task LoadDataAsync()
    {
        ...
        try
        {
            ...
            {
                // await Dispatcher.UIThread.InvokeAsync(() => MainViewModel.StatusMessage = "Database connection failed.");
                return;
            }
            
            var sets = await _dbContext.AttributeSets.AsNoTracking().ToListAsync();
            var attributes = await _dbContext.Attributes.AsNoTracking().ToListAsync();
    ...
    private async Task CreateFromTemplate()
    {
        ...
        try
        {
            if (await _dbContext.AttributeSets.AnyAsync(s => s.Id == newId))
            {
            ...
            _dbContext.AttributeSets.Add(newSet);
    ...
    private async Task RenameCategoryAsync(AttributeCategoryViewModel? categoryVm)
    {
        ...
        var preview = await _dbContext.PreviewAttributeChangeAsync(dummyOldId, dummyNewId, categoryVm.CategoryName, newName);
    ...
    private async Task DeleteCategoryAsync(AttributeCategoryViewModel? categoryVm)
    {
        ...
        referencingSets = await _dbContext.GetReferencingAttributeSetsAsync(attributeIds);
        if (attributeIds.Any())
        {
            affectedValuesCount = await _dbContext.AttributeValues
                .AsNoTracking()
