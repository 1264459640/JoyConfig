...
public partial class AttributeDatabaseContext : DbContext
{
    ...
    public virtual DbSet<JoyConfig.Infrastructure.Models.AttributeDatabase.Attribute> Attributes { get; set; }
    ...
    public async Task ChangeAttributeSetIdAsync(string oldId, string newId)
    {
        await using var transaction = await Database.BeginTransactionAsync();
        try
        {
            // Step 1: Create a new AttributeSet with the new ID
            var originalSet = await AttributeSets.AsNoTracking().FirstOrDefaultAsync(s => s.Id == oldId);
            if (originalSet == null)
            {
                throw new Exception($"AttributeSet with ID '{oldId}' not found.");
            }
            ...
            // Step 2: Update all referencing AttributeValues
            await Database.ExecuteSqlInterpolatedAsync($@"
                UPDATE AttributeValues
                SET AttributeSetId = {newId}
                WHERE AttributeSetId = {oldId};");

            // Step 3: Delete the old AttributeSet
            await Database.ExecuteSqlInterpolatedAsync($@"
                DELETE FROM AttributeSets
                WHERE Id = {oldId};");
        ...
        {
            await transaction.RollbackAsync();
            throw new Exception($"A transactional error occurred while changing the attribute set ID. The operation was rolled back. Details: {ex.Message}", ex);
        }
    }

    public async Task<(bool IsValid, string? ErrorMessage)> ValidateDatabaseSchemaAsync(string? dbPath)
    {
